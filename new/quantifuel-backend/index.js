...
let MongodbState = 0;
let connectedPumps = {}; // Stores connected pump sessions
...
// WebSocket connection for pumps
io.on('connection', (socket) => {
  print.log('Pump connected:', socket.id);
    ...
    try {
      ...
      if (!pump_id) {
        socket.emit('handshake_response', {
          status: 'FAIL',
          message: 'Missing pump_id',
        });
        return;
      }

      // Store pump connection
      connectedPumps[pump_id] = socket.id;
      print.success(`Handshake successful for Pump ${pump_id}`);

      // Send acknowledgment to the pump
      socket.emit('handshake_response', { status: 'ACK' });
    } catch (error) {
      print.error('Error in handshake:', error);
      socket.emit('handshake_response', {
        status: 'FAIL',
  ...
  socket.on('disconnect', () => {
    Object.keys(connectedPumps).forEach((pump_id) => {
      if (connectedPumps[pump_id] === socket.id) {
        print.log(`Pump ${pump_id} disconnected.`);
        delete connectedPumps[pump_id];
      }
    });
  });
});
